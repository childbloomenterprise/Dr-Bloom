// Layout + visual primitives ported from bloom-primitives.jsx
'use client';
...
export function Stack({
  children, gap = 12, style, dir = 'col', align, justify,
}: { children?: React.ReactNode; gap?: number; style?: CSS; dir?: 'col' | 'row'; align?: CSS['alignItems']; justify?: CSS['justifyContent'] }) {
  return <div style={{ display: 'flex', flexDirection: dir === 'row' ? 'row' : 'column', gap, alignItems: align, justifyContent: justify, ...style }}>{children}</div>;
}

export function HRow({
  children, gap = 10, style, align = 'center', justify,
}: { children?: React.ReactNode; gap?: number; style?: CSS; align?: CSS['alignItems']; justify?: CSS['justifyContent'] }) {
...
export function Card({
  children, p, tone = 'surface', radius = 'lg', style, onClick,
}: { children?: React.ReactNode; p?: number; tone?: CardTone; radius?: keyof typeof T.radius; style?: CSS; onClick?: () => void }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      color: tone === 'brand' ? '#fff' : T.ink900, ...style,
    }}>{children}</div>
  );
}
...
export function Button({
  ...
  const s = sizes[size];
  const styles: CSS = variant === 'primary'
  ...
  return (
    <button type={type} disabled={disabled} onClick={onClick} style={{
      ...
      fontSize: s.fs, fontWeight: 600, letterSpacing: T.sansTracking, cursor: disabled ? 'not-allowed' : 'pointer',
      width: full ? '100%' : 'auto', flexShrink: 0, opacity: disabled ? 0.6 : 1, ...styles, ...style,
    }}>
...
export function Chip({ children, tone = 'wash', icon, style }: { children?: React.ReactNode; tone?: 'wash' | 'cream' | 'soft' | 'accent' | 'surface'; icon?: string; style?: CSS }) {
  ...
  return (
    <div style={{
      ...
      fontFamily: T.sans, fontSize: 12, fontWeight: 500, ...style,
    }}>
    ...
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color || T.brand} strokeWidth={stroke}
          strokeLinecap="round" strokeDasharray={`${C * value} ${C}`} transform={`rotate(-90 ${size / 2} ${size / 2})`} />
      </svg>
      {label != null && (
...
export function BloomFlower({
  size = 200, petals = [0.85, 0.7, 0.78, 0.6, 0.9, 0.72],
}: { size?: number; petals?: number[] }) {
...
export function Input({
  ...
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <input {...rest} style={{
        height: 48, padding: '0 16px', borderRadius: T.radius.pill, border: `1px solid ${T.line}`,
        background: T.surface, color: T.ink900, fontFamily: T.sans, fontSize: 14,
        letterSpacing: T.sansTracking, outline: 'none', ...(rest.style || {}),
      }} />
    </label>
  );
}
...
export function ProgressBar({ value = 0.5, color, h = 6 }: { value?: number; color?: string; h?: number }) {
  return (
    <div style={{ height: h, background: T.ink100, borderRadius: T.radius.pill, overflow: 'hidden', width: '100%' }}>
      <div style={{ height: '100%', width: `${Math.max(0, Math.min(1, value)) * 100}%`, background: color || T.brand, borderRadius: T.radius.pill }} />
    </div>
  );
}
