...
export function TopBar({ eyebrow, title, subtitle, trailing, isMobile, onMenu }: Props) {
  if (isMobile) {
    return (
      <div style={{ padding: '14px 18px', background: T.bg, borderBottom: `1px solid ${T.line}`, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={onMenu} style={{
            width: 40, height: 40, borderRadius: T.radius.pill,
            background: T.surface, border: `1px solid ${T.line}`,
            display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.ink900, flexShrink: 0,
          }}>
            <Icon name="menu" size={17} stroke={1.7} />
          </button>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 26, height: 26, borderRadius: T.radius.pill, background: T.brand, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
  ...
  return (
    <div style={{ padding: '24px 32px 18px', background: T.bg, display: 'flex', alignItems: 'flex-end', gap: 20, borderBottom: `1px solid ${T.line}`, flexWrap: 'wrap' }}>
